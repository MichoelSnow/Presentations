...
## Import the data
citi_bike <- read_csv('C:/citibike_data/201701-citibike-tripdata.csv') # Make sure to change the directory to match where you extracted the file
citi_bike

#### You can see that R solved the problem of spaces in the column names by putting \` around them
#### (note this is a backtick not a single quote), but this makes it annoying to refer to these variables.  
#### Instead we can use a snake case version of the column names as an input into `read_csv`
column_names <- c('trip_duration','start_time', 'stop_time', 'start_id', 'start_name', 'start_lat',  
                  'start_long', 'end_id', 'end_name', 'end_lat', 'end_long', 'bike_id', 'user_type', 'birth_year', 'gender')
citi_bike <- read_csv('./citibike_data/201701-citibike-tripdata.csv',col_names = column_names)
citi_bike
#### Use the `skip` parameter to skip the first row, i.e., the column names, since we are using our own names
citi_bike <- read_csv('./citibike_data/201701-citibike-tripdata.csv',col_names = column_names, skip=1)
citi_bike

## Writing csv files
csv_write(citibike,"C:/Path/to/file/name.csv")
