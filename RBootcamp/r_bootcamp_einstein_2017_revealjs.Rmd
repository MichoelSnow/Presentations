...
options(width = 999)
knitr::opts_chunk$set(tidy.opts=list(width.cutoff=130),width=130,tidy=TRUE)
```
...
```{r}
JanDelay <- filter(flights,month==1)
summarise(JanDelay,delay = mean(dep_delay,na.rm = TRUE))
```
...
- Using the babynames dataset from the babynames package:
    1. How popular was your name in the US in the year you were born
    2. Which are the overall most popular girl and boy baby names as measured by count? How many times more popular were they then the next most popular names?
    3. Which 5 years had the most babies named "Michael"? How about the greatest proportion of babies named "Michael"?
    4. Since 1990 how many girls have been named "Michael"
    5. Which name had the biggest increase in consecutive years and in what years was it?
