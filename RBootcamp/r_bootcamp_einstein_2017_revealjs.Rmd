...
I forgot about flights which were delayed so left the next morning, and since dep_time is day agnostic, this does not give me the data I am looking for.  I can try and figure out which flights left that day and which left the next or just use a different variable as my metric of the late evening flights which left earliest.
```{r}
...
```{r, out.width = "700px",echo=FALSE}
knitr::include_graphics("select.gif")
```
...
```{r}
flights_sml <- select(flights, dep_time,arr_time, air_time, distance) # create a subset of the full dataset so that you can see new variables being added
flights_sml
