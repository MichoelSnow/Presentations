...
\begin{equation}
  Var(\bar{x}) = s^2 = \frac{1}{n-1}\sum\limits_{i=1}^n \left(x_i - \bar{x}\right)\left(x_i - \bar{x}\right)  = \frac{1}{n}\sum\limits_{i=1}^n \left(x_i - \bar{x}\right)^2 
  (\#eq:SmpVarDisc)
\end{equation}
...
\begin{equation}
  Var(x) = \sigma^2 = \frac{1}{n}\sum\limits_{i=1}^n \left(x_i - \mu_x\right)\left(y_i - \mu_y\right)
  (\#eq:PopCovDisc)
\end{equation}

If equation \@ref(eq:PopCovDisc) looks suspiciously like equation \@ref(eq:PopVarDisc), that's because the formula for the variance is just a special case of the formula for the covariance when dealing with a single variable.  To get an intuitive sense of what the covariance represents here are a 
...
x1 <- sort(runif(ct,-10,10))
x2 <- sort(x1,decreasing = TRUE)
y1 <- sort(runif(ct,-10,10))
y2 <- sort(x1,decreasing = TRUE) 
var_2d <- tibble(x=c(x1,x2,x1,x2),
                 y=c(y1,y1,y2,y2),
                 Variances=c(rep("x1y1",ct),rep("x2y1",ct),rep("x1y2",ct),rep("x2y2",ct)))
ggplot(var_2d) + geom_point(aes(x,y,color=Variances),size=2) 
  # scale_color_discrete(labels=c(round(var(x1),2),round(var(x2),2),round(var(x3),2)))
```
...
# true_ct <- 10^3
smp_ct <- 10^1
# true_rain <- runif(true_ct,0,10)
true_rain <- seq(0,10,0.001)
true_ct <- length(true_rain)
smp_rain <- sample(true_rain,smp_ct)
true_var <- 1/true_ct*sum((true_rain-mean(true_rain))^2)
smp_var <- 1/smp_ct*sum((smp_rain-mean(smp_rain))^2)
unb_var <- var(smp_rain)
```
...
\begin{equation}
  var(x) = \sigma^2 = \frac{1}{N}\sum\limits_{i=1}^N \left(x_i - \bar{x}\right)\left(x_i - \bar{x}\right)  = \frac{1}{N-1}\sum\limits_{i=1}^N \left(x_i - \bar{x}\right)^2 
  (\#eq:Var)
\end{equation}

Where $\bar{x}$ is the notation for the mean value of x.  The reason why the variances is the sum of squares divided by $N-1$ and not $N$ is because, I am calculating the variance based on the sample mean and not the population mean.   


The **covariance** is a measure of the joint variability between two variables.  If they tend to grow and shrink together, the covariance is positive, and if one grows as the other shrinks then the covariance is negative.  The equation for the covariance is very similar to that for variance, as variance is just a special case of covariance with a single variable: 


\begin{equation}
  var(x) = \sum\limits_{i=1}^N \left(x_i - \bar{x}\right)\left(y_i - \bar{y}\right)  
  (\#eq:Covar)
\end{equation}

In order to get an intuitive understanding of covariance I think it is 

I want to go through a few simpler relationships which will come back later in the derivation.
...
If you are not familiar with it, the initial equation is the equation for the variance of a variable.  The variance is a measure of spread for a single variable, i.e., the distance of each value from the average value.  You are most probably familiar with variance when discussing normal or Gaussian distributions, where the variance gives the 

```{r, echo=FALSE}
norm_smp <- tibble(a = rnorm(10^6),b=rnorm(10^6,sd=2)) 

norm_smp %>% ggplot() + geom_density(aes(b,..scaled..),color="blue",fill="blue",alpha=0.1) + 
  geom_density(aes(a,..scaled..),color="red",fill="red",alpha=0.2)

```



If instead of using the same variable in both halves of the initial equation there were different variables, this would be the equation for the covariance.  I can do similar manipulation with the covariance as follows:
...
This series of equations is simply going back and forth between equivalent values using the relationships established in equation \@ref(eq:SumMn).  I can split and rejoin summations because the operations are all linear.  At the end of the above series I end up with two relationships which will be needed later.  If you are not familiar with it, the initial equation is the equation for the covariance 
