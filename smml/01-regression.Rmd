...
The general idea of the least squares method is that you want to pick coefficents which minimize the differences between the given output and the output calculated from the right hand side of equation \@ref(eq:SimpLinReg).  What makes this process least squares is that you calculate the difference using the sum of the squared differences, termed the residual sum of sqaures (RSS).  Formally the residual sum of squares for the general form of linear regression in equation \@ref(eq:LinReg) can be written as :
